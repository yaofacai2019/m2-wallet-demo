...
The hosted demo can be published as a static, installable sandbox. Each browser
receives isolated local data without touching another visitor's data or any real
digital assets.
...
本地进程会生成临时 API Key 并打印在终端；也可通过环境变量
`M2_WALLET_API_KEY` 显式设置。监听地址和端口可使用 `M2_WALLET_HOST`、
`M2_WALLET_PORT` 配置。
...
- `GET|POST /api/v1/payment-fee-rules`（修改仅限管理员）
